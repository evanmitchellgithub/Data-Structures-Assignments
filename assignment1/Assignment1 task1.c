...
/* 
This is the code for Assignment 1, part 1
*/
...
void insert_function( int key,  int x){
    ...
    if(hash_array[key].key == -999){ // checks if the key slot is free 
        ...
        while(loopcount < array_size){
            ...
            if(hash_array[tempkey].key == key && (strcmp(hash_array[tempkey].surname, surnames[x].surname) == 0)){
                ...
                //printf("\n name : %s", hash_array[tempkey].surname);
                //printf("| key : %d\n", hash_array[tempkey].key);
                hash_array[tempkey].frequency++;
                num_terms++; 
                return;     
            }
